...
export function getDataByBox(url, that) {
  let tname = null;
  switch (that.mqpf) {
    case 'mqf' : tname = `qpf_r03_${that.tname}`;
      break;
    case 'temp' : tname = `oefs_tmp_${that.tname}`;
      break;
    case 'uv' : tname = `oefs_eda10_${that.tname}`;
      break;
  }
  http({
    method: 'post',
    url: `${showserver}/rest/grb2/real/getWhr`,
    data: {'coor': that.coor, 'curpage': that.curpage, 'pagesize': that.pagesize, 'tname': tname},
    timeout: 60000
  }).then(response => {
