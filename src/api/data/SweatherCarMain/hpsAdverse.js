...
// 删除marker
export function removeMarker(param, that) {
  // if (markers!=null){
  console.log(markers)
  // }
...
export function getFeature(param, that) {
  ...
  axios.post(`${showserver}/rest/waiapp2/selectFeatureByTime`, `wtype=${param}&bounds=${coor}`, {headers: {'Content-Type': 'application/x-www-form-urlencoded'}}).then(res => {
    res = res.data;
    switch (param) {
      case 10800:
...
export function getFeatureCity(param, that) {
  http({
    url: `${showserver}/rest/waiapp2/selectCitysGroup?wtype=${param}`,
    method: 'get'
  }).then(res => {
    if (res.code !== 0) return;
