        ...
        <el-collapse accordion v-model="activeNames">
          <el-collapse-item v-for="(itm, idx) in sideWeather" :key="idx" :name="itm.type">
            <template slot="title">
              ...
              <!-- <el-checkbox-group v-model="checkList" @change="adverseChange(itm.flag,itm.label)">
                <el-checkbox style="margin: 0 0 0 10px;" :label="idx" :key="idx"></el-checkbox>
              </el-checkbox-group> -->
         ...
         <div class="Typhoon-hzy">
           <el-checkbox v-model="typhoonFlag"  style="margin: 0 0 0 10px;float: left;"></el-checkbox>
      ...
      <el-row :gutter="20" style="min-width: 100%;margin-left: 0px; overflow:hidden">
        ...
        <div style="display: inline-block;position: absolute; top: 10px;left: 300px;right: 0; overflow:hidden">
          <el-row :gutter="10" style=" overflow:hidden">
            ...
            <!-- 台风 -->
            <div style="float: right;margin-right:10px;width: 116px;height:87px;margin: 0 0 10px 0;padding-left: 55px; white-space: nowrap;background-image: url('static/icon/weather/freeze_gray.png');background-repeat: no-repeat;
                background-size: contain; box-shadow: 0 2px 12px 0 rgba(0,0,0,.1);
                border: 1px solid #EBEEF5">
                <div style="height: 85px;padding-left: 0;padding-right: 0">
                  <p class="p_wgrade_lb" style="font-size: 14px"><label class="wgrade_lb">总台风数:</label></p>
                  <p class="p_wgrade_lb"><label class="wgrade_lb">普通台风:</label></p>
                  <p class="p_wgrade_lb"><label class="wgrade_lb">强力台风:</label></p>
                  <p class="p_wgrade_lb"><label class="wgrade_lb">超强台风:</label></p>
                </div>
              </div>
          </el-row>
        </div>
      </el-row>
      <el-row style="position: absolute; top: 125px;left: 16px; right: 16px; bottom: 16px; z-index: 100; overflow:hidden; " :gutter="20">
              ...
              <el-table :data="_filterRightData(rightFromData)" style="width: 100%;height: 100%;overflow: auto"
                ...
                <el-table-column prop="name" class-name="table_cell2">
                  <template slot-scope="scope">
                    ...
                    <p v-if="scope.row.objectGrade === '10801'">危险: 车辆泡水，挪车躲避</p>
                    <p v-if="scope.row.objectGrade === '10802'">危险: 刹车缓顿，停车躲避</p>
                    <p v-if="scope.row.objectGrade === '10803'">危险: 视野受限，减速慢行</p>
                    <p v-if="scope.row.objectGrade === '10901'">危险: 道路飓风，挪车躲避</p>
                    <p v-if="scope.row.objectGrade === '10902'">危险: 道路强风，停车躲避</p>
                    <p v-if="scope.row.objectGrade === '10903'">危险: 道路横风，减速慢行</p>
                    <p v-if="scope.row.objectGrade === '11001'">危险: 冰雹一级，挪车躲避</p>
                    <p v-if="scope.row.objectGrade === '11002'">危险: 冰雹二级，停车躲避</p>
                    <p v-if="scope.row.objectGrade === '11003'">危险: 冰雹三级，减速慢行</p>
...
<script>
  ...
  import {map, loadMap, getFeature, getFeatureCity, togglePopup, showPolygon, clearMarkers} from 'src/api/data/SweatherCarMain/hpsAdverse';
